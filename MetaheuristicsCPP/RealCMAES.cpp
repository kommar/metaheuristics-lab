...
CCMAESIndividual::CCMAESIndividual(IEvaluation<double, double> &cEvaluation, VectorXd &vSample)
...
{
	d_fitness = c_evaluation.tEvaluate(v_genotype);
}//void CCMAESIndividual::vEvaluate()


CRealCMAES::CRealCMAES(IEvaluation<double, double> &cEvaluation, IStopCondition &cStopCondition, mt19937 &cRandomEngine, double dInitSigma)
	: COptimizer<double, double, CSingleObjectiveOptimizationResult<double>>(cEvaluation, cStopCondition, new CSingleObjectiveOptimizationState<double>(cEvaluation.tGetMaxValue())), v_means(cEvaluation.iGetSize()), v_previous_means(v_means), m_covariance_matrix(cEvaluation.iGetSize(), cEvaluation.iGetSize()), d_init_sigma(dInitSigma), c_random_engine(cRandomEngine), v_p_sigma(cEvaluation.iGetSize()), v_p_c(cEvaluation.iGetSize())
{
...
bool CRealCMAES::b_run_iteration(long long iIterationNumber, clock_t tStartTime)
{
	...
	return b_update_state(*v_population.front());
}//bool CRealCMAES::b_run_iteration(long long iIterationNumber, clock_t tStartTime)
...
bool CRealCMAES::b_update_state(CCMAESIndividual &cIndividual)
{
	return COptimizer<double, double, CSingleObjectiveOptimizationResult<double>>::b_update_state(cIndividual.vGetGenotype(), cIndividual.dGetFitness());
}//bool CRealCMAES::b_update_state(CCMAESIndividual &cIndividual)
